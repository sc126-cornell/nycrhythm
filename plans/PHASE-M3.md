...
| 項目 | 內容 |
|---|---|
| 狀態 | 🟢 已結案（2026-07-20 用戶實地 Validate 通過） |
| 預估 | 1.5 人日（實際約 0.3） |
...
- [x] Gate 前雙掃描（2026-07-20 結案時執行：未勾項僅餘模板佔位與 BACKLOG A2 觀測項）

## 4. Validate — 需求驗收

- [x] 用戶實測（含遞延的 M2 實地對板一併）：2026-07-20 多趟紐約地鐵實測——**操作順暢；對板誤差最長 30 秒內、大部分幾乎同步**
- [x] 同意進 M4/M5（實際時序：用戶 7/19 已指示先行，M4/M5 同日完成；本格於實地驗證後補結）
...
### Validate 結果
**2026-07-20 用戶實地通過**：多趟紐約地鐵實測，操作順暢；對板（M2 遞延項合併驗）誤差最長 30 秒內、大部分幾乎同步——RT-first 架構的預測品質獲實地確認。實地回饋衍生的修正（速度尖刺、站名、圓點、光束）已於 R1／R2／F2 系列同步收斂。
...
- [x] 已回報 M3 結果並取得確認（日期：2026-07-20，用戶：「操作順暢。時間非常準，最長誤差30秒內……兩個驗證可以關掉」）
- [x] Gate 前雙掃描已執行（2026-07-20）
