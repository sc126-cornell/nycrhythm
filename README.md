...
- **Engine** (`web/`): each train is interpolated along official track geometry between its own real-time stop predictions. Follow mode, station arrival boards (transfer-complex aware), search, deep links, your GPS position with compass heading (📍, on-device only). Auto day/night follows the NYC sun.
