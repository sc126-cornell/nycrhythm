# BACKLOG — nycrhythm

> M4/M5 收尾時的遞延與前瞻總表（2026-07-19）。

## A. 驗收遞延（時間到就做）

| # | 項目 | 時點 |
|---|---|---|
| A1 | ~~實地對板（M2）＋M3 體驗~~ ✅ 通過（多趟實測：操作順暢、誤差 ≤30s、多數同步） | 2026-07-20 |
| A2 | 平日尖峰效能觀測（600+ 班 fps）——**7/21 06:52 實測 695 班上圖**；伺服器端 8 餵送全綠（age 4–12s、合計 715 班、單路 327–845ms）；殘：用戶 `?fps=1` 幀率讀數 | 週一（2026-07-21）早高峰 |
| A3 | ~~轉 Public~~ ✅ 完成（用戶指令；gh 確認 PUBLIC/MIT） | 2026-07-19 |

## B. 技術強化

| # | 項目 | 說明 |
|---|---|---|
| B1 | og:image 分享圖 | 1200×630（雙城同款缺） |
| B2 | tripId→shape 模糊直配 | 現 52% 直配＋48% defaults 墊底（100% 覆蓋）；前綴比對可拉高精準變體率 |
| B3 | 餵送健康監測 | 8 餵送個別失效的降級提示（現為整體 stale 標記） |
| B4 | VehiclePositions 加細 | currentStatus（進站/停靠/離站）修飾插值端點 |
| B5 | 單元測試 | position/rt 引擎（現靠管線驗證器＋實測） |
| B6 | eco 判斷 | UA 嗅探 → 能力偵測（承捷奏 backlog） |
| B7 | spike-static.ts 更名 | CI 借用 spike 腳本下載 zip——語義整理為 fetch-static.ts |

## D. post-v1 已出貨功能

| 日期 | 功能 | 備註 |
|---|---|---|
| 2026-07-19 | 📍 我的位置（用戶提議） | 頂欄切換鈕；watchPosition 藍點＋精度圈；首次定位置中一次、跟車自動降 free；NYC 80km 外不移鏡頭；定位資料只在裝置上、不外傳（BUILD F1） |
| 2026-07-19 | 🧭 朝向光束＋開站自動定位（用戶提議） | 藍點加 Google Maps 式方向扇形；iOS webkitCompassHeading＞Android absolute alpha＞GPS course（步行中）；角度解纏繞避免轉長路。開站即自動啟動定位（用戶指示）：深連結開站不搶鏡頭、auto 模式錯誤／NYC 外靜默；iOS 羅盤權限依規需手勢→首次觸碰畫面時補綁（已授權者無感）（BUILD F2） |
| 2026-07-20 | 👆 看板列車可點選（用戶提議） | 到站看板每列加 data-trip＋事件委派：點列＝選取該車次（同點地圖 bullet）——收看板、開資訊卡、鏡頭移至該車（z≥14）；頁腳提示 tap a train to select it（BUILD F3） |

## C. 前瞻（不承諾）

Alerts 橫幅（MTA alerts 餵送）、時間旅行（靜態班表重播）、PWA、多城市引擎抽象（等雙城都穩定運轉後再議）、LIRR/Metro-North 擴充

## 流程備忘

- Gate 前雙掃描（checkbox＋待填）；Check 紀錄與實作同日填寫
- 背景 dev server 用完即收（2026-07-19 用戶手機看到 2 running tasks 的教訓）
